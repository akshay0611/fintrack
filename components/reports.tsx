...
export function Reports({ className, dateRange }: ReportsProps) {
  ...
  const groupedExpenses = useMemo(() => {
    const filtered = transactions.filter((expense: any) => {
      ...
      return (!dateRange?.from || expenseDate >= dateRange.from) && (!dateRange?.to || expenseDate <= dateRange.to)
    })
        ...
        <ChartContainer config={{ amount: { label: "Amount", color: "hsl(var(--primary))" } }} className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 20, right: 20, bottom: 20, left: 40 }}>
              ...
              <YAxis axisLine={false} tickLine={false} width={60} tickFormatter={(value) => `₹${value}`} tick={{ fill: "hsl(var(--muted-foreground))" }} />
              <ChartTooltip content={<ChartTooltipContent className="bg-background/95 backdrop-blur-sm" />} formatter={(value) => [`₹${Number(value).toFixed(2)}`, "Amount"]} cursor={{ fill: 'hsl(var(--muted))', opacity: 0.2 }} />
