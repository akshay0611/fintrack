...
interface EditSubscriptionFormProps { subscription: { id: string; name: string; amount: number; cycle: string; start_date: string; next_renewal_date: string; status: string; notes: string } }
...
export function EditSubscriptionForm({ subscription }: EditSubscriptionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) { toast.error(result.error) } else { toast.success("Subscription updated successfully!"); setOpen(false) }
