...
export default function SubscriptionsPage() {
  ...
  const handleExportCSV = () => {
    ...
    const csvContent = [['Name', 'Amount', 'Cycle', 'StartDate', 'Status'].join(','), ...subscriptions.map((s: any) => [s.name, s.amount, s.cycle, s.start_date, s.status].join(','))].join('\n');
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-b from-background to-background/95">
      <div className="fixed left-0 top-0 h-screen w-12 border-r bg-card/50 backdrop-blur-sm"><SideNav /></div>
